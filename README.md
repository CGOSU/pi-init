...
精确编辑的连续操作可复用当前上下文中的逻辑快照：首次缺少新鲜目标内容时先读取，成功 edit 后且期间没有其他写入来源时，后续 edit 不强制重复读取。执行可能写入目标文件的命令或工具、Git 操作，或发现外部协作者修改后，必须重新读取。若 exact-text 因 `oldText` 零匹配失败，只能定向重读、重新确认唯一精确替换并最多重试一次；不得使用模糊匹配、正则或仅凭行号猜测。逻辑快照不生成缓存文件、备份文件或其他持久状态。
