...
- 使用 `pi-usage --receipt` 可生成参照每日 AI 对账单样式的纵向 SVG 图片；日期参数仍沿用普通报表，使用 `--receipt-output <路径>` 指定输出文件，未指定时生成当前目录下可预测的 `pi-usage-receipt-<日期>.svg`。图片中的费用、模型、Token、缓存命中率和会话数均来自同一份 summary；SVG 不增加图像渲染运行时依赖，也不等同于实际账单。
