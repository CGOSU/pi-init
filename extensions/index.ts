...
export default function initProjectExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use task_workflow action=plan only for an explicit planning request, cross-module or high-risk work, or a plan that cannot be safely handled as a small local change; in workflowMode=auto, do not create a plan merely to confirm known facts or for ordinary low-risk work that fits at most two role/dependency tasks. Route clear instructions directly to the matching role; if the instruction is unclear, ambiguous, or crosses responsibilities, start with architect. If the uncertainty is repository evidence, code location, implementation, or call-chain detail, architect must first switch to docs-commit for minimal structured evidence.",
