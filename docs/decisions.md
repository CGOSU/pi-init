...
### 2026-09-22：pi-usage 对账单采用无依赖 SVG 输出

- 决定：在 `pi-usage` 中通过通用的 `--output <路径>` 将现有查询 summary 输出为中文纵向 SVG 对账单，不增加独立的账单模式参数；时间范围完全复用普通查询的 positional 参数。
- 原因：查询和导出应共享同一份结果与时间语义，避免维护第二套日期入口；SVG 可以直接作为可分享图片打开，且不需要引入 PNG/Canvas 运行时依赖，不破坏现有跨平台启动器和 DuckDB 运行时边界。
- 约束：`--output` 目前只接受 `.svg` 路径；费用、模型、Token、缓存命中率和会话数只能来自真实 summary；对账单明确标注 API 等值估算、非实际账单；未指定输出时默认文本报表保持不变，动态文本必须 XML 转义。

### 2026-09-21：测试默认只验证高价值稳定契约

- 决定：测试和断言默认不锁定纯文案、标题、提示语、样式、布局、渲染结构、简单组件存在性、内部临时字段、简单映射和框架行为；优先验证权限安全、核心规则、状态流转、边界、数据一致性、幂等并发、公共 API 和历史缺陷。
- 原因：文案与表现层细节变化频繁，负向文案断言和样式断言不能改变产品行为，却增加执行时间和维护成本；测试应降低真实回归风险，而不是增加测试数量。
- 约束：删除展示文字时直接删除旧断言，不以 `doesNotMatch` 替代；exactly-once、防重复派发、公共协议、稳定错误语义、生成模板关键规则和工具元数据可作为稳定契约保留。此次已清理相关测试，未修改生产行为。
...
- 原因：首次消息延迟可能来自扩展前置处理、Prompt/工具准备、Provider 首 token、模型持续生成、Provider 多轮往返、工具调用、自动重试或 settled 收尾；只记录总 Agent 时长无法区分这些阶段。诊断钩子只记录时间，不阻断、改写或额外触发 Agent 回合。
...
- 约束：活动工作流、隐藏续跑、取消和中断不生成普通完成提示；无效时间戳不能伪造耗时，非 TUI 行为不改变。

### 2026-09-15：控制中心根菜单采用四个顶层分组并独立换行说明

- 决定：将控制中心根菜单提升为“初始化”“变更”“同步”“工作流”四项；初始化和变更进入各自次级菜单，同步和工作流直接执行对应操作。TUI 选择菜单使用尽可能宽的弹窗，并把当前选中项的说明移到列表下方，通过文本组件自动换行。
- 原因：减少根菜单中重复的动作前缀，降低层级和横向文本竞争；窄终端下仍能查看完整操作说明，而不是依赖单行截断。
- 约束：Esc 仍按菜单层级逐级返回；角色配置的 Ctrl+S 保存行为、初始化表单的返回语义和非 TUI 命令入口不改变。

### 2026-09-15：模板同步采用托管区块，角色配置统一使用 Ctrl+S

- 决定：新增 `/pi-init sync [目录]`，只同步带标记的模板区块并以 `.pi/pi-init-state.json` 保存 schema 与基线；已有项目记忆文档只保留或创建，不覆盖历史。控制中心所有角色配置菜单通过 `Ctrl+S` 调用统一保存回调，移除可见保存列表项，保留 `/pi-init save` 兼容入口。
- 原因：将模板升级与首次初始化分离，避免覆盖项目定制内容；将保存动作从菜单列表项解耦，确保根菜单和嵌套配置菜单行为一致。
- 约束：托管区块本地修改或无法精确识别时必须报告冲突并保持原文件；保存失败或重复按键不得关闭菜单或并发写入；初始化表单不承担角色配置保存语义。

### 2026-09-15：项目脚手架生成 Fast Path 会话收尾约束

- 决定：将 Fast Path 收尾优先级、默认跳过例行文档同步和验证、文档目标例外、退出条件及用户显式要求例外写入中英文 `templates/AGENTS.md`，使 `createScaffold` 生成的项目规则直接具备该约束。
- 原因：这是 pi-init 生成项目规则的产品行为，不应只依赖维护者本机的全局 `AGENTS.md` 或当前仓库的协作文件。
- 约束：不改变公共角色 Skill 的职责边界；生成规则仍要求保持仓库一致性，遇到 API、架构、业务、权限、路由、交互或无障碍语义变化时退出 Fast Path。
...
- 原因：防止架构角色在机械取证、执行或外部交互中偏离其决策和编排定位；将事实核验、实现验证和状态修改交给对应职责，降低误用工具和职责越界风险。
- 约束：`architect` 不修改文件、不执行命令、不连接 MCP，也不直接完成、阻塞、重试、取消或恢复工作流；简单只读咨询和低风险开发由适合的非 architect 角色直接处理。现有 roleModels、工作流状态机和其他角色职责不变；已安装 package 和当前 Pi 进程需更新并 reload/重启后才加载本次规则。
- 验收：实现和验证结果记录于 [`docs/session-log.md`](session-log.md)，本决策不重复维护验证明细。
- 替代：本条替代 2026-09-09“按风险分级放宽 AI 自主决策与 architect 只读边界”中允许 architect 低风险只读探索的部分；该条关于低风险自主决策、真实验证和高风险边界的其他约束仍有效。
...
- 原因：简单任务的目标是快速完成实现，避免全量验证和工作流编排带来的不必要耗时；直接相关的最小核对仍保留，以避免把未检查的修改描述为已验证。
- 约束：不得因此跳过安全措施、输入校验、数据不丢失处理、无障碍基础和用户明确要求的验证；涉及公共接口、权限、数据、并发、迁移、删除、共享工作区或其他高风险边界时恢复必要的针对性验证，无法安全判断时先交给 architect。
- 影响：该策略只改变低风险任务的默认验证范围，不修改 `task_workflow`、`subtask`、`parallel_batch` 的结果协议、完成条件或恢复边界；交付必须明确说明未执行的验证和剩余风险。

### 2026-09-10：采用 gmc worktree 作为并行批次的隔离层

- 决定：并行开发第一版由 Pi 主会话负责任务规划、角色路由、结果验收和工作流状态；由 gmc 负责创建、列举、回收和管理独立 Git worktree；并行任务以批次执行，默认最多同时运行 2 个 worker，批次完成后由主会话串行集成并执行全量验证。
- 原因：复用 gmc 已有的 worktree 能力，避免恢复 pi-init 已移除的自研隔离 worker；同时保留 pi-init 的职责边界、严格验证和单一状态写入者。
- 约束：worker 必须绑定固定基线、任务 ID、独立 worktree 和不重叠文件范围；worker 不得直接写主工作流状态、提交、推送或强制清理；共享 `.env`、`node_modules`、数据库和构建输出默认不作为跨 worktree 的可变共享状态；集成失败必须阻塞批次而不能猜测完成；不直接采用 gmc 默认的绕过审批/沙箱命令。
- 验收：覆盖至少两个真正独立任务的并行执行、单 worker 失败接管、重复结果/过期结果拒绝、主会话恢复后不重复派发、串行集成冲突处理，以及 Windows 宿主上的真实端到端验证。

### 2026-09-09：项目记忆文档的时间记录统一采用倒序

- 决定：`session-log.md`、`decisions.md` 和 `pitfalls.md` 的日期条目按日期倒序排列，最新条目在前；`current-state.md` 仅“最近一次更新”列表按倒序排列，当前事实正文不按时间排列；`clean-code.md` 等静态规则文档不适用。
- 原因：打开文档即可看到最新工作、决策和已知风险，同时保留当前状态与历史记录的语义边界。
- 约束：新增条目插入对应日期位置；同日没有可靠时间依据时保留既有相对顺序，不补造时间；其他既有项目不会通过脚手架或 package 更新自动重排其本地历史记录。

### 2026-09-09：按风险分级放宽 AI 自主决策与职责编排

- 决定：在用户已确认目标内，低风险、局部、可逆且不改变既定契约的技术决策由 AI 自主完成，不把 helper 选择、内部拆分、测试组织、排查顺序或已有模式内的实现选项转交用户；只读咨询和一般代码分析不强制创建工作流或进行正式职责交接。
- 决定：只有业务目标冲突、公共契约/数据/权限/架构含义无法从现有证据确定、不可逆或外部状态操作、无法安全合并已有改动、缺少必要权限/凭据或真实验证阻塞时才暂停询问；询问必须给出推荐方案和最少必要问题。
- 决定：复杂、高风险、跨职责工作仍保留结构化证据、角色边界、真实验证和授权 Git 规则；取消的是低风险场景的流程仪式，不取消安全、正确性和验证边界。
- 原因：当前全局 Skill 发现范围、项目 AGENTS、角色路由和 architect 工具守卫叠加后，简单分析也可能触发 docs-commit/architect 往返，且一般技术选择被模型误判为用户决策，降低开发速度。
- 约束：architect 仍不得修改文件；只读探索可在低风险分析中直接进行，修改工具、初始化写入、外部状态写入和高风险证据门仍受守卫；`workflowMode`、task_workflow 协议和完成验证语义不改变。
- 替代：本条关于低风险只读探索和按需交接的规则，替代 2026-09-05“架构师只负责分析决策，证据探索交给文档与提交工程师”中对所有探索一律前置交接的部分；该条关于 architect 不写文件、复杂证据分工和职责边界的其余约束继续有效。

### 2026-09-07：角色与编排规则按单一层级精简

- 决定：公共 Skill 作为路由语义唯一入口；明确职责直接路由，不明确、含糊或跨职责从 `architect` 开始，缺少仓库证据时由 `docs-commit` 取证后返回 `architect`。角色说明只保留职责、边界和交接，运行时提示只保留当前任务必需的硬约束。
- 原因：角色划分本身足够清晰，重复维护完整证据门控、切换、编辑和完成规则会增加上下文噪声与维护漂移；集中语义、局部执行约束可以降低冗余而不改变行为。
- 约束：保留 `architect`、`developer-test`、`docs-commit` 及 `roleModels` 配置；保留真实验证、`complete`/`block`、高风险新鲜证据、敏感信息、授权 Git、恢复门和 architect 工具守卫；不修改 `task_workflow` API、任务 schema、状态机、`workflowMode` 或执行器。

### 2026-09-07：不明确的编排指令默认从架构师开始

- 决定：编排时，能够明确对应某个职责的指令直接从对应角色开始；无法明确归类、需求含糊或涉及跨职责的指令默认从 `architect` 开始。
- 原因：先由架构师统一澄清目标、判断边界并决定后续职责，避免不明确请求被误路由到开发或文档角色。
- 约束：如果只是代码位置、实现、调用链或证据不清，架构师仍不得自行探索，必须先切换到 `docs-commit` 收集最小结构化证据；明确对应角色的指令不额外经过架构师；不改变既有角色职责边界和工作流状态机。

### 2026-09-06：任务规划采用依赖与风险优先、同层先易后难的软排序

- 决定：任务规划按“用户明确优先级与硬依赖 → 可能推翻方案的关键未知项 → 业务关键路径 → 同层且风险与价值相近时先易后难”的顺序编排；先易后难仅作为软约束。
- 原因：保留用户意图和依赖关系，尽早验证会导致返工的关键未知项，同时在条件相近时优先交付较容易验证的任务，避免严格按难度排序延后关键问题。
- 约束：用户明确的任务顺序、截止要求和 `dependsOn` 不得被难度排序覆盖；关键未知项必须拆成可验收、限时的最小验证，不得演变为无限调研；不新增 difficulty/risk 字段，不修改 `task_workflow` API、任务 schema、状态机、roleModels 或执行器，也不自动改写用户提供的任务输入顺序。

### 2026-09-05：工作流阻塞必须显示原因和恢复建议

- 决定：工作流因任务阻塞进入 `blocked`/`paused` 后，状态报告、TUI 弹窗、`task_workflow` 工具结果和暂停通知统一显示具体阻塞原因，并给出 retry 或 replan 的建议解决方法。
- 原因：仅显示“已暂停”无法帮助用户判断下一步；阻塞原因和可执行恢复路径应在所有工作流展示入口保持一致。
- 约束：阻塞原因继续使用状态机记录的真实 `blockReason`，缺失的历史状态明确显示未记录，不猜测原因；解决阻塞后使用 `/pi-init workflow retry <taskId>`，需求或方案变化时由架构师使用 `task_workflow(action="replan")`。

### 2026-09-05：架构师只负责分析决策，证据探索交给文档与提交工程师

- 决定：架构师职责限定为理解问题、分析根因、建立逻辑关系、做关键决策、确定方案和制定执行计划；搜索、浏览、读取、定位、调用链追踪、测试/文档查找和上下文整理等证据收集由文档与提交工程师先行完成并以结构化证据交接。
- 原因：避免架构师把时间和上下文预算消耗在机械探索上，使架构输出集中在判断、取舍和计划；同时由专责角色统一整理来源、路径、调用方、测试和不确定项，减少遗漏与重复搜索。
- 约束：架构师不得自行执行文件/仓库/命令探索，不得把未经证据角色核实的猜测写成事实；若证据不足或发现需求/边界冲突，必须让文档与提交工程师补充证据或返回用户决策。架构师仍可进行纯思考、分析、决策、计划以及必要的角色/工作流交接。
- 约束：文档与提交工程师可读取和追踪代码、配置、测试及项目文档，但不修改代码；开发测试工程师负责实现、测试和验证；文档与提交工程师在实现完成后负责文档同步、diff 检查及经用户授权的提交/推送。
