...
- 决定：在 `pi-usage` 中新增 `--receipt` 模式，基于现有 summary 生成中文纵向 SVG 对账单；通过 `--receipt-output <路径>` 指定文件，未指定时使用可预测的当前目录文件名。
- 原因：SVG 可以直接作为可分享图片打开，且不需要引入 PNG/Canvas 运行时依赖，不破坏现有跨平台启动器和 DuckDB 运行时边界。
- 约束：费用、模型、Token、缓存命中率和会话数只能来自真实 summary；对账单明确标注 API 等值估算、非实际账单；默认文本报表输出保持不变，动态文本必须 XML 转义。
