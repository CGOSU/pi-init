...
- 决定：精确编辑首次缺少新鲜内容时先读取；成功 `edit` 后，在没有其他写入来源时复用已确定的替换结果作为当前上下文中的逻辑快照；`exact-text` 因 `oldText` 零匹配失败时定向重读并最多安全重试一次。
- 原因：减少同一会话连续编辑同一文件时的重复读取，同时保留精确唯一替换和冲突停止语义；逻辑快照只属于当前上下文，不声称底层工具提供缓存或自动推断修改意图。
- 约束：命令、工具、Git 操作或外部协作者可能写入目标文件后，逻辑快照立即失效并必须重读；多重匹配、重叠替换、文件删除/重命名、编码问题、其他错误或第二次失败都必须停止。首版不引入完整文件内容缓存、hash/revision、文件监听、缓存文件、持久状态、配置项或运行时依赖。
