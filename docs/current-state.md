...
- `parallel_develop` 工具在架构规划后，在允许自动切换且项目受信任时为多个开发测试任务创建隔离 worktree；子代理通过 Pi JSON 事件流实时报告任务状态、当前工具、耗时和最后活动，基础设施错误自动重试一次，代码/测试错误交由主开发测试工程师接管，失败现场和日志保留，全部成功后才合并和清理；当前已有确定性 worktree、合并、重命名范围、心跳和重试测试。
- 默认映射为 `gpt-5.6-sol/max`、`gpt-5.6-luna/max`、`gpt-5.6-luna/medium`，项目可覆盖。
- 支持简体中文、英文、dry-run 和已有文件覆盖确认。
- 初始化可选择默认职责配置或为每个职责选择当前可用模型与兼容的推理强度；运行中 `/role-config` 可持久修改职责映射并立即应用，Pi 原生 `/model` 与 `Shift+Tab` 仍是会话级临时切换。
- 测试命令为 `npm test`。
...
- 2026-08-06：增加并行子代理实时事件、心跳、基础设施重试和失败现场保留；同时增加可配置职责模型、初始化选择和运行时持久配置命令。
