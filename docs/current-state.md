...
- 非工作流的 `interactive`/`rpc` 外部输入从首次 `agent_start` 计时到最终 `agent_settled`，完成后写入不进入 LLM 上下文的 `pi-init-run-timing` custom entry，工作报告继续显示每轮耗时；TUI 工作时使用 Pi 原生 `Working`，空闲后在编辑器上方显示 `─ Worked for ... ─`，累计本次 session 的 Agent 实际工作时间且不计闲置时间。session 恢复时优先从每轮完成后保存的独立累计快照恢复，并兼容只有普通执行记录的旧 session；活动工作流、扩展隐藏续跑和未完成/中断执行不补造普通报告。
