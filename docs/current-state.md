...
- 自动模式在真实跨角色且上下文使用率达到 50% 时，于当前回合结束后触发一次定制上下文压缩；成功后注入隐藏续跑消息，失败仅提示并保留已切换角色。会话启动、resume 或 reload 时，会根据当前模型和推理强度唯一匹配角色并恢复角色状态。
...
- 测试命令为 `npm test`。
- 提供跨平台 `scripts/pi-fast.*` 和 `scripts/pi-update.*` 启动器；Windows PowerShell 安装器会把 `.cmd` 文件复制到 Pi 所在的 npm 可执行目录，POSIX 安装器优先使用 Pi 可执行目录、无写权限时回退到用户 bin 目录。
...
- 2026-08-06：增加自动跨角色上下文压缩；统一 `/pi-init` 控制中心，增加状态卡片、分组菜单、首次使用引导、响应式摘要、即时模型能力提示、紧凑通知和返回上一级交互；并行子代理默认并发调整为 2，增加高频事件节流、真实阶段/worker 指标和 `terminated` 模型错误识别重试；生成的中英文 Skill 增加精确字符串替换说明。
