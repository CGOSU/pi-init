...
- TUI 中“工作流 · 查看任务进度”以及 `/pi-init workflow status` 现在打开居中 overlay 弹窗，使用主题背景色、标题高亮和四边框明确区分弹窗，显示状态、进度、总任务开始时间、执行器、规划、暂停原因和可滚动任务列表；已完成任务条目显示任务耗时；RPC 等非 TUI 模式的状态文本也显示总任务开始时间和已完成任务耗时。
