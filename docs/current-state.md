...
- 非工作流的 `interactive`/`rpc` 外部输入从首次 `agent_start` 计时到最终 `agent_settled`，完成后写入不进入 LLM 上下文的 `pi-init-run-timing` custom entry；TUI 空闲时以 `─ Worked for ... ─` 横向分隔线显示本次执行耗时。活动工作流、扩展隐藏续跑和未完成/中断执行不补造普通记录，完整来源、时间戳和计时口径仍保留在 session entry 中。
