...
- 控制中心现在显示模式、职责、模型状态卡片，按“初始化/职责”分组菜单，首次进入提供简短引导；取消配置会返回上一级菜单，初始化通知默认只显示文件数量和冲突摘要。
- 模型选择在 TUI 中使用带即时筛选的搜索列表，显示模型名称和支持的推理级别，并使用友好的职责和模式名称；Pi 原生 `/model` 与 `Shift+Tab` 仍是会话级临时切换。
- 测试命令为 `npm test`。
