...
- 工作流启用并创建任务后，每个任务完成时，开发测试工程师必须实际执行验证，并调用 `task_workflow` 的 `complete` 动作提交摘要和真实结果；完成时输出精简任务报告，包含任务、角色、开始/结束时间、总耗时、摘要和验证结果。最终工作流报告同样保持精简；报告时间统一显示为东八区 `YYYY-MM-DD HH:mm:ss+08:00`。工作流会自动推进、自动切换到任务指定角色并开始下一个可执行任务。
