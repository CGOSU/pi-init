...
- After each task in an enabled workflow, the Development and Test Engineer must run real verification and call `task_workflow` with `complete`, including the implementation summary and actual results. Completion must also output a task completion report containing the task ID, task, role, start time, end time, total duration, completion summary, and verification results. The workflow automatically switches to the assigned role and starts the next ready task.
