...
- Model references must be explicit: role models and `subtask` workflow configuration use full `provider/model` values and require exact registered references. Native Agent subagents use the host's model selection; pi-init does not inject, validate, or intercept their `model` parameter. To change a role's provider, edit the role model directly (`/pi-init config` lists every registered model and `/pi-init save` persists, or edit `.pi/role-models.json`).
...
- During an active workflow, describe a changed direction or new follow-up work in ordinary natural language; consecutive ordinary `interactive`/`rpc` inputs during the same task are merged in arrival order into one pending revision with a `revisionId`, never into multiple revisions. The extension pauses the old plan only when the current task reaches its boundary. The Architect uses the complete merged instruction set to submit the unfinished-work plan with `task_workflow(action="replan")`; no old follow-up task starts before the new plan is applied, and completed tasks, summaries, and verification records are immutable. To stop the current task immediately, use the existing `/pi-init workflow cancel` flow.
