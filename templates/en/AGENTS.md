...
- Model references must be exact: the main session and Agent subagents use full `provider/model` values. An omitted Agent `model` inherits the current full model; fuzzy names such as `haiku` or `sonnet` without `provider/` are rejected before spawn; a specified model must exist exactly, with no fuzzy matching or cross-provider fallback. To change providers, edit the role model directly (`/pi-init config` lists every registered model and `/pi-init save` persists, or edit `.pi/role-models.json`).
