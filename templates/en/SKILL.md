...
- After receiving a task, the Development and Test Engineer should implement, test, and fix directly instead of pausing for optional preferences. On completion, call `task_workflow(action=complete, taskId=..., completionSummary=..., verification=[...])` and output a concise task report containing the task, role, start/end time, total duration, summary, and verification results. The final workflow report is concise as well, retaining only the goal, progress, task summaries, overall timing, duration, and verification. The verification list may contain only commands actually run and their real results; report times use the system local timezone in `YYYY-MM-DD HH:mm:ss±HH:MM` format.
- The executor is configured by top-level `workflowExecutor` in `.pi/role-models.json` and defaults to `local`; with `subtask`, the main session delegates the current task to a separate conversation fork by calling the `subtask` tool, and the workflow advances when the fork's result message returns to the session. A missing tool or a malformed result must safely block the task.
- During an active workflow, submit changed directions or new follow-up work in ordinary natural language; consecutive ordinary `interactive`/`rpc` inputs during the same task are merged in arrival order into one pending revision with a `revisionId`, never into multiple revisions. The extension pauses the old plan only at the current task boundary. Only the Architect may use the complete merged instruction set to submit the unfinished-work plan with `task_workflow(action="replan")`; no old follow-up task starts before the new plan is applied, and completed tasks, summaries, and verification records are immutable. Use the existing `/pi-init workflow cancel` flow to stop the current task immediately.
- With `subtask`, a revision does not make pi-init terminate or re-dispatch a running fork, and it must not start the old plan's next task; the fork must be inspected or stopped manually when needed.
- Model references must be explicit: role models, `switch_role`, `/pi-init config`, and `subtask` workflow configuration use full `provider/model` values and require exact registry matches. Native Agent subagents use the host's model selection; pi-init does not inject, validate, or intercept their `model` parameter. To use another role provider, pick any registered model via `/pi-init config` (staged anytime, persisted by `/pi-init save`) or edit `.pi/role-models.json` directly.
- With `subtask`, the parent session is the sole writer of `task_workflow` state. The fork only executes the current task, never calls `task_workflow`, and must return strict JSON using `pi-init/task-result@1`; only a valid `complete` result may complete a task.
- The fork works in the shared checkout; it must not create worktrees, merge branches, commit, or push automatically. After reload, a delegated non-terminal task is not re-dispatched automatically; inspect the state and recover manually.
...
- After completion, the extension switches the configured role/model and starts the next task through a hidden continuation message unless a pending revision exists; with a revision, it first hands the unfinished plan to the Architect and must not start an old follow-up task. Do not manually reassign the next task or ask the user to trigger it. If the model forgets the completion action, the system nudges it a limited number of times and then pauses.

## Role Switching Modes

- The top-level `mode` in `.pi/role-models.json` can be `auto`, `confirm`, or `manual`; the default is `auto`.
- `auto` applies automatic role changes immediately; `confirm` asks before an automatic change, with “Accept suggestion” selected by default and options to switch to manual mode or cancel; `manual` blocks automatic changes, native `/model` switches are never rolled back by the extension, and each switch writes the active role model straight back to `.pi/role-models.json`; `/pi-init role <role ID>` still switches roles manually.
...
- In Pi 0.84, `model_select` is a post-selection notification. The extension immediately restores the previous or configured safe model and rechecks policy at `session_start`, input, and before provider requests; a future cancellable `before_model_select` event can move this rejection to the selection source.
