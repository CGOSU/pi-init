...
- Model references must be explicit: role models, `switch_role`, `/pi-init config`, and fully qualified Agent/subtask models use `provider/model` values. An omitted Agent `model`, or an unqualified name such as `haiku`/`sonnet`, inherits the current full model with a notice; a fully qualified model must exist exactly in the registry, with no cross-provider fallback. To use another provider, pick any registered model via `/pi-init config` (staged anytime, persisted by `/pi-init save`) or edit `.pi/role-models.json` directly.
