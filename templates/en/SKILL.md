...
- Provider policy is configured by top-level `providerPolicy` in `.pi/role-models.json`, defaulting to `{"mode":"locked","allowedProviders":["openai-codex"]}`; legacy projects without the field remain locked by default. Model selection, cycling, restore, role/workflow switches, and Agent spawns must use an allowed provider. An omitted Agent `model` inherits the current model; unqualified names such as `haiku`/`sonnet` and other provider/model values are rejected before spawn. Cross-provider use requires an explicit saved config edit; there is no temporary unlock or implicit fallback. The only exception is manual mode: with `mode: "manual"` every guard is bypassed to face the host directly, and a native `/model` switch writes the active role model back to `.pi/role-models.json`, appending the new provider to the allowlist when needed.
...
- `auto` applies automatic role changes immediately; `confirm` asks before an automatic change, with “Accept suggestion” selected by default and options to switch to manual mode or cancel; `manual` blocks automatic changes and faces the host directly: provider guards are bypassed, native `/model` switches are not rolled back, and each switch writes the active role model (adding the provider to the allowlist when needed) straight back to `.pi/role-models.json`; `/pi-init role <role ID>` still switches roles manually.
