...
- During an active workflow, submit changed directions or new follow-up work in ordinary natural language; the extension records a revision with a `revisionId` and pauses the old plan only at the current task boundary. Only the Architect may submit the unfinished-work plan with `task_workflow(action="replan")`; completed tasks, summaries, and verification records are immutable. Use the existing `/pi-init workflow cancel` flow to stop the current task immediately.
...
- Model references must be exact: role models, `switch_role`, `/pi-init config`, Agent spawns, and subagent delegation all use full `provider/model` values. An omitted Agent `model` inherits the current full model; unqualified names such as `haiku`/`sonnet` are rejected before spawn; a specified model must exist exactly in the registry, with no fuzzy matching or cross-provider fallback. To use another provider, pick any registered model via `/pi-init config` (staged anytime, persisted by `/pi-init save`) or edit `.pi/role-models.json` directly.
