...
2. 简单只读咨询可直接交给当前适合的非 `architect` 角色完成，不创建工作流，也不要求正式证据交接；目标明确的日常开发由 `developer-test` 一次完成调查、修改和验证。
...
- 用户明确要求简单任务只做实现时，采用最小验证策略：不创建 `task_workflow`，不启动 `subtask`/`collaboration`，不默认运行全量测试、类型检查或构建；纯文档、文案、注释和样式修改只做必要的差异/静态核对。不得因此跳过安全措施、输入校验、数据不丢失处理、无障碍基础或用户明确要求的验证；涉及公共接口、权限、数据、并发、迁移、删除或共享工作区等高风险边界时恢复针对性验证，并明确列出未执行项。
...
- 活动工作流的普通方向变更在当前任务边界合并为一个 revision；应用新计划前不得启动旧后续任务。委派 Agent 不写主工作流状态、不创建 worktree、不合并、不提交或推送，并返回严格的 `pi-init/task-result@1` 结果。
- `workflowExecutor: "collaboration"` 使用共享 cwd、独立 Agent 进程/session、Agent registry、消息和文件 reservation；任务必须绑定 pi-init role，子 Agent 使用 roleModels 的精确 provider/model/thinkingLevel，不得从第三方 type/TOML 回退或覆盖。
- reservation 只协调 `edit`/`write` 工具的路径所有权，不是文件系统或 shell 沙箱；共享工作区的失败/取消可能留下部分修改，不能伪造回滚。启动、完成、失败、取消、reload、session replacement 和 shutdown 必须保持真实状态，迟到结果不得推进旧任务。
